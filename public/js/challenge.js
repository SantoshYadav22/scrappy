...
function reset_dish_list() {
    ...
    $("#filter_challenge_id").val('');
    $("#filter_name").val('');
    $("#filter_challenge").val('');
    $("#is_active").val('');
...
function dish_list() {
    ...
    var filter_challenge_id = $("#filter_challenge_id").val();
    var filter_name = $("#filter_name").val();
    var filter_challenge = $("#filter_challenge").val();
    var is_active = $("#is_active").val();
  

    console.log(filter_challenge_id,filter_name,filter_challenge,is_active);
...
function load_more(page1) {
    ...
    console.log(filter_challenge_id);
    ...
    $.ajax({
        url: API_URL + "challenge/list?page=" + page,
        type: "get",
        datatype: "html",
        data: {
            filter_challenge_id,
...
function dish_details(challenge_id) {
    // $(".alice-blue").removeClass('alice-blue');
    // $("#employee_head_" + challenge_id).addClass('alice-blue');
    ...
    document.getElementById(`employee_head_${challenge_id}`).style.backgroundImage = "linear-gradient(120deg, #84fab0 0%, #8fd3f4 100%)";
    $.ajax({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
        },
        method: "POST",
        url: API_URL + "challenge/details",
...
// function handleDeleteDeish(challenge_id, status) {
...
//                 },
//                 method: "POST",
//                 url: API_URL + "hallenge/change-status",
//                 data: {challenge_id: challenge_id, status: status},
//                 success: function (res) {
//                     // var obj = JSON.parse(res);
//                     console.log(res);
...
//     });
// }
function get_edit_dish(challenge_id) {
...
function get_single_dish_list(challenge_id) {
    $.ajax({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
        },
        method: "post",
        url: API_URL + "challenge/single-data",
        data: {challenge_id: challenge_id},
        success: function (res) {
            $("#task_list_" + challenge_id).empty().html(res);
