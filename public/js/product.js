...
function load_more(page1) {
    ...
    var filter_pro_name = $("#filter_pro_name").val();
    var filter_status = $("#filter_status").val();
   
    console.log(filter_pro_name);
    ...
    $.ajax({
        url: API_URL + "product/list?page=" + page,
        type: "get",
        datatype: "html",
        data: {
            filter_cat_id,
...
function dish_list() {
    ...
    console.log(filter_product,filter_cat_id,filter_pro_name,filter_status);
...
function dish_details(prod_id) {
    // $(".alice-blue").removeClass('alice-blue');
    // $("#employee_head_" + prod_id).addClass('alice-blue');
    ...
    document.getElementById(`employee_head_${prod_id}`).style.backgroundImage = "linear-gradient(120deg, #84fab0 0%, #8fd3f4 100%)";
    $.ajax({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
        },
...
function get_edit_dish(prod_id) {
...
function get_single_dish_list(prod_id) {
    $.ajax({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
        },
        method: "post",
        url: API_URL + "product/single-data",
        data: {prod_id: prod_id},
        success: function (res) {
            $("#task_list_" + prod_id).empty().html(res);
...
function handleDeleteDeish(prod_id, status) {
        ...
        swal({
            ...
            if (isConfirm) {
                $.ajax({
                    ...
                    method: "POST",
                    url: API_URL + "product/change-status",
