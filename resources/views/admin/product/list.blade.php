...
@if(isset($data['pageno']) && $data['pageno'] == 0 && $key == 0)
    <input type="hidden" value="{{ $val['prod_id'] ?? '' }}" name="first_chef_dish_id" id="first_chef_dish_id">
@endif
<?php
    $status = array("In-Active","Active");
?>
<input type="hidden"   value="{{ $val['prod_id'] ?? '' }}" name="first_chef_dish_id" id="first_chef_dish_id">
<div class="employee-box main-card mb-3 mt-2 card" id="task_list_{{ $val['prod_id'] ?? '' }}" style="cursor: pointer;" onclick="dish_details('{{ $val['prod_id'] ?? '' }}')">
    <div class="employee-head card-header remve_Active_color d-flex justify-content-between"
        id="employee_head_{{ $val['prod_id'] ?? 'N/A' }}">
        <div class="name">
            <span class="value">{{ $val['prod_name'] ?? 'N/A' }} ({{$val['prod_id'] ?? ''}})</span>
        </div>
        <div class="id" >
    ...
    <div class="detail justify-content-between p-2 ">
        <div class="corporate_logics_sub d-flex justify-content-between  align-items-center flex-wrap mb-2">
            ...
            <div class="">
                ...
                <span class="value">{{ $val['cat_id'] ?? 'N/A' }}</span>
            </div>
